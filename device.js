...
const sw_07 = document.getElementById("sw_07");
const sw_08 = document.getElementById("sw_08");
...
const dev_07 = document.getElementById("dev_07");
const dev_08 = document.getElementById("dev_08");
...
database.ref("/TT_IoT3/Light").on("value", function (snapshot) {
    let a = snapshot.val();
    if (a == 0) {
...
database.ref("/TT_IoT3/Power").on("value", function (snapshot) {
    let a = snapshot.val();
    if (a == 0) {
        dev_07.style.backgroundColor = "#252530"; // Thay bằng màu nền mặc định
        sw_07.checked = false;
    } else {
        dev_07.style.backgroundColor = "#5edcb5"; // Thay bằng màu chủ đạo
        sw_07.checked = true;
    }
});

database.ref("/TT_IoT3/Door").on("value", function (snapshot) {
    let a = snapshot.val();
    if (a == 0) {
        dev_08.style.backgroundColor = "#252530"; // Thay bằng màu nền mặc định
        sw_08.checked = false;
    } else {
        dev_08.style.backgroundColor = "#5edcb5"; // Thay bằng màu chủ đạo
        sw_08.checked = true;
    }
});
...
sw_05.onclick = function () {
    if (sw_05.checked) {
        ...
        database.ref("/TT_IoT3").update({ "Light": 1 });
    } else {
        console.log("false");
        dev_05.style.backgroundColor = "#252530";
        database.ref("/TT_IoT3").update({ "Light": 0 });
    }
};
...
sw_07.onclick = function () {
    if (sw_07.checked) {
        console.log("true");
        dev_07.style.backgroundColor = "#5edcb5";
        database.ref("/TT_IoT3").update({ "Power": 1 });
    } else {
        console.log("false");
        dev_07.style.backgroundColor = "#252530";
        database.ref("/TT_IoT3").update({ "Power": 0 });
    }
}

sw_08.onclick = function () {
    if (sw_08.checked) {
        console.log("true");
        dev_08.style.backgroundColor = "#5edcb5";
        database.ref("/TT_IoT3").update({ "Door": 1 });
    } else {
        console.log("false");
        dev_08.style.backgroundColor = "#252530";
        database.ref("/TT_IoT3").update({ "Door": 0 });
    }
}