...
database.ref("/TT_IoT3/Curtains").on("value", function (snapshot) {
    let a = snapshot.val();
    if (a == 0) {
...
database.ref("/TT_IoT3/camera").on("value", function (snapshot) {
    let a = snapshot.val();
    if (a == 0) {
...
sw_02.onclick = function () {
    if (sw_02.checked) {
        ...
        database.ref("/TT_IoT3").update({ "Curtains": 1 });
    } else {
        console.log("false");
        dev_02.style.backgroundColor = "#252530";
        database.ref("/TT_IoT3").update({ "Curtains": 0 });
    }
};
...
sw_06.onclick = function () {
    if (sw_06.checked) {
        ...
        database.ref("/TT_IoT3").update({ "camera": 1 });
    } else {
        console.log("false");
        dev_06.style.backgroundColor = "#252530";
        database.ref("/TT_IoT3").update({ "camera": 0 });
    }
}